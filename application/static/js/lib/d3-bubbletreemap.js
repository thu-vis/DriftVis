(function (global, factory) {
    ...
    function getLayerClusters(hierarchyRoot, layerDepth, padding) {
        ...
        // console.log("node depth: ", layerNodes.map(d=>d.depth));
        layerNodes.forEach(function (fnode) {
            let clusterNodes = fnode.descendants().filter(function (candidate) {
                return !candidate.children;
            });

            let clusterParent = fnode.ancestors().filter(function (ancestor) {
                return ancestor.depth === layerDepth;
            })[0];

            let tmp = fnode.ancestors().filter(function (ancestor) {
                return ancestor.depth === layerDepth;
            });
            // console.log("ancestors: ", fnode.id, tmp.map(d=>d.id));
            ...
            clusterNodes.forEach(function (node) {
                ...
                // console.log("uncertaintySum:", fnode.id, node.id, node.path(clusterParent),
                //     path, path.map(d=>d.uncertainty),
                //     clusterParent.id, uncertaintySum);
                ...
                let planckClusterParentUncertainty = clusterParent.uncertainty;      // Padding for force based layout (contours should not cut each other, i.e. full parent contour should be taken into account).

                // let interClusterSpacing = clusterNodes.length === 1 ? 0 : padding / 2.0; // For single circle: No padding, since it has no contour.
                let interClusterSpacing = padding / 2.0; // For single circle: No padding, since it has no contour.
    ...
    function generatePath(outerCircleRing, curvature) {
        let arcs = [];
        for (let i = 0; i < outerCircleRing.length; i++) {
            ...
            let larger_r = Math.sqrt(centerToFirstIntersection.x * centerToFirstIntersection.x +
                centerToFirstIntersection.y * centerToFirstIntersection.y);
            ...
            // console.log("arcStartAngle: ", arcStartAngle, "arcEndAngle: ", arcEndAngle);
            ...
            let cur_arcEndAngle = new Vec2(0, -1).angle(intersectionToFirstCenter);
            let cur_arcStartAngle = new Vec2(0, -1).angle(intersectionToSecondCenter);
            if (cur_arcStartAngle > cur_arcEndAngle) {
                cur_arcStartAngle -= Math.PI * 2;
            }
            // console.log("cur_arcStartAngle: ", cur_arcStartAngle, "cur_arcEndAngle: ", cur_arcEndAngle);

            // console.log("centerToFirstIntersection:", centerToFirstIntersection, circle);
            arcs.push({
                "x": centerToFirstIntersection.x * r / larger_r + circle.center.x,
                "y": centerToFirstIntersection.y * r / larger_r + circle.center.y,
                "rx": circle.center.x,
                "ry": circle.center.y,
                "r": circle.radius,
                "large_arc": Math.abs(arcEndAngle - arcStartAngle) > Math.PI ? 1 : 0,
                "clickOrder": 1
            });
            arcs.push({
                "x": centerToSecondIntersection.x * r / larger_r + circle.center.x,
                "y": centerToSecondIntersection.y * r / larger_r + circle.center.y,
                "rx": intersection.x,
                "ry": intersection.y,
                "r": curvature,
                "large_arc": Math.abs(cur_arcEndAngle - cur_arcStartAngle) > Math.PI ? 1 : 0,
                "clickOrder": 0
            })
        }
        // create path according to arcs
        let path = "M " + arcs[0].x + " " + arcs[0].y + " ";
        for (let i = 0; i < arcs.length; i++) {
            let from_point_x = arcs[(i) % arcs.length].x;
            let from_point_y = arcs[(i) % arcs.length].y;
            let to_point_x = arcs[(i + 1) % arcs.length].x;
            let to_point_y = arcs[(i + 1) % arcs.length].y;
            let rx = arcs[(i) % arcs.length].rx;
            let ry = arcs[(i) % arcs.length].ry;
            let circle_r = arcs[(i) % arcs.length].r;
            let large_arc = arcs[(i) % arcs.length].large_arc;
            let click_order = arcs[(i) % arcs.length].clickOrder;
            let s = "a " + circle_r + " " + circle_r + " " + "0 " + large_arc + " " + click_order + " " +
                (to_point_x - from_point_x) + " " + (to_point_y - from_point_y) + " ";
            path = path + s;
        }
        // console.log("path", path);
        return path;
    ...
    function contourInOnePath(nodes, nodes_parent, curvature) {
        if (nodes.length === 1) {
            // console.log("circle node", nodes[0].children, nodes[0].id);
            return {
                "path": _circle_path(nodes[0].x, nodes[0].y, nodes[0].r +
                    (nodes_parent.children ? nodes[0].contourPadding : 0)),
        ...
        let outerCircleRing = getOuterCircleRing(circles, curvature);
        // console.log("outerCircleRing", outerCircleRing);

        // return [{"1":1}];
        return {
            "path": generatePath(outerCircleRing, curvature),
            "center": [0, 0]
        };
    }
    ...
    function getOuterCircleRing(circles, curvature) {
        ...
        // console.log("circles", circles,"circlesEnlarged", circlesEnlarged, "curvature", curvature);
    ...
    function contourHierarchyById(hierarchyRoot, padding, curvature) {
        let contours = {};
        // console.log("hierarchyRoot: ", hierarchyRoot.height);
        for (let layerDepth = hierarchyRoot.height; layerDepth >= 0; layerDepth--) {
            // Get clusters of circles on this layer.
            let layerClusters = getLayerClusters(hierarchyRoot, layerDepth, padding);
            // console.log("layerCluster: ", layerClusters);
            // Create contour for each cluster.
            layerClusters.forEach(function (cluster) {
                let generatedContour = contourInOnePath(cluster.nodes, cluster.parent, curvature);
                ...
                // });
                // contours = contours.concat(generatedContour);
                // console.log(cluster.parent.id, generatedContour);
                contours[cluster.parent.id] = generatedContour;
            });
        }
        return contours;
    }
    ...
    function bubbletreemap() {
        ...
        return bubbletreemap = {
            ...
            getContourById: function () {
                return contourHierarchyById(hierarchyRoot, padding, curvature);
            },
