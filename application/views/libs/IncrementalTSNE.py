...
class IncrementalTSNE:
    ...
    def fit_transform(self, X, _y=None, skip_num_points=0, constraint_X=None, constraint_Y=None, alpha=0,
                      label=None, corrected_id=None, corrected_weight=None, init_id=None, labels=None, constraint_labels=None, gmm=None, label_alpha=0.6, label_beta=0, constraint_weight=None):
        """
        ...
        if label is not None:
            label = label.astype(np.int32, copy=True)
        ...
        embedding = self._fit(X, skip_num_points=skip_num_points, constraint_X=constraint_X, constraint_Y=constraint_Y,
                              alpha=alpha, label=label, corrected_id=corrected_id, corrected_weight=corrected_weight, init_id=init_id, labels=labels,
                              constraint_labels=constraint_labels, gmm=gmm, label_alpha=label_alpha, label_beta=label_beta, constraint_weight=constraint_weight)
        self.embedding_ = embedding
        return self.embedding_

    def _fit(self, X, skip_num_points=0, constraint_X=None, constraint_Y=None, alpha=0, label=None, corrected_id=None, corrected_weight=None, init_id=None, constraint_labels=None, gmm=None, labels=None, label_alpha=1, label_beta = 0, constraint_weight=None):
        """
        ...
        if self.method == "exact":
            ...
            distribution = gmm['distribution']
            means = {i: distribution[i]['mu'] for i in distribution}
            n_labels = len(means)
            N, D = X.shape
            labels = np.concatenate((labels, [i for i in distribution]))
            means_embed = {i: np.zeros(self.n_components) for i in distribution}
            ...
            #print('embedding', Y[:100])
            for i in range(N):
                means_embed[labels[i]] = means_embed[labels[i]] + Y[i]
            for i in distribution:
                means_embed[i] /= distribution[i]['n']
            means = list(means.values())
            means_embed = list(means_embed.values())
            cffi_verbose = 0
            if self.verbose:
                cffi_verbose = 1
            if labels is None:
            ...
            if len(distribution) == 1:
                label_alphas = [label_alpha]
            else:
                # print('log_dets', [distribution[key]['log_det'] for key in distribution])
                log_dets = []
                for i in distribution:
                    while i >= len(log_dets):
                        log_dets.append(0)
                    det = distribution[i]['log_det']
                    log_dets[i] = 0.5 * det + D * 0.5 * (1 + math.log(2 * math.pi))
                max_log_det = max(log_dets)
                label_alphas = [(max_log_det - label_beta * x) / (max_log_det) * label_alpha for x in log_dets]
                # print('entropys', log_dets)
                # print('alpha', label_alphas)
            #print(distribution)
                    ...
                    try:
                        ...
                        for j in range(1, len(ret[1])):
                            if labels[i] == labels[ret[0][j]]:
                                dist.append(ret[1][j] * label_alphas[labels[i]])
                            else:
                                dist.append(ret[1][j])
                        distances.append(dist)
                    except:
                        print('len_X', X.shape[0], i, labels[i])
                        print('len_labels', len(labels))
                        print('label_alpha', len(label_alphas))
                        distances.append(ret[1][1:])
            ...
            if isinstance(constraint_X, np.ndarray):
                # np.savetxt('X.txt', X)
                # np.savetxt('Y.txt', Y)
                # np.savetxt('constraint_X.txt', constraint_X)
                # np.savetxt('constraint_Y.txt', constraint_Y)
                # exit(1)
            ...
            if constraint_N > 0:
                ...
                #print('constraint_distances', constraint_distances)
                #print('constraint_indices', constraint_indices)
            ...
            if isinstance(label, np.ndarray) and isinstance(corrected_id, np.ndarray) and isinstance(corrected_weight, np.ndarray) and isinstance(init_id, np.ndarray):
                ...
                cffi_label = self.ffi.cast('int*', label.ctypes.data)
            ...
            t = FuncThread(self.C.run_bhtsne, cffi_X, N + n_labels, D, cffi_Y, self.n_components,
                           ...
                           cffi_constraint_distances, cffi_constraint_weight)
            ...
            return ret[:N], (means, ret[N:], [(distribution[i]['n'] ** 0.5) for i in distribution])
